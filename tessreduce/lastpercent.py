...
def _find_bkg_cor(tess,cores):
    ...
    cor = Parallel(n_jobs=cores, backend=getattr(tess, 'backend', 'loky'), verbose=(1 if getattr(tess, 'verbose', 1) >= 2 else 0))(delayed(_parallel_correlation)
...
def multi_correlation_cor(tess, limit=0.8, cores=7):
    ...
    results = Parallel(n_jobs=cores, backend=getattr(tess, 'backend', 'loky'), verbose=(1 if getattr(tess, 'verbose', 1) >= 2 else 0))(
